...
class Tools{
	public static function print_screen($current_plugin_version)
	{
		...
		$errors = self::checkFile(SCRIPT_ROOT);
	 ...
	 */
	public static function compareHashes(array $hashes, $rootDir)
	{
		$cwd = getcwd();
		chdir($rootDir);
	...
	public static function checkFile($rootName){
		...
		$errors = self::compareHashes($hashes, $rootName);
	...
	public static function get_mobiquo_config()
	{
		$config_file = SCRIPT_ROOT.'mobiquo/config/config.txt';
	...
	public static function checkAddonVersion(){
		...
		$config = self::get_mobiquo_config();
